...
const VendorOrderRow = React.memo(function VendorOrderRow({
  order,
  currency,
  onPress,
}: {
  ...
  const itemCountLabel = useMemo(() => {
    const count = sumQty(order.itemsForThisVendor ?? []);
    return count === 1 ? '1 item' : `${count} items`;
  }, [order.itemsForThisVendor]);

  const thumbnailUrls = useMemo(() => {
    return (order.itemsForThisVendor ?? []).map((i) => i.imageUrl);
  }, [order.itemsForThisVendor]);
...
export default function VendorOrders() {
  ...
  const { data: orders, isLoading, isFetching, error, refetch } = useGetVendorOrdersQuery(undefined, {
    pollingInterval: isFocused ? 12000 : 0,
    refetchOnFocus: true,
  ...
  const renderItem = useCallback(
    ({ item }: { item: VendorOrderListItem }) => (
      <View style={{ marginBottom: theme.spacing.md }}>
        <VendorOrderRow order={item} currency={currency} onPress={openOrder} />
      </View>
    ),
        ...
        {isLoading ? (
          ...
          <Card style={{ gap: theme.spacing.sm }}>
            ...
            <Text style={{ color: theme.colors.placeholder, fontWeight: '600' }}>
              {filter === 'all' ? 'Incoming orders will appear here.' : 'Try a different filter or check back later.'}
            </Text>
          </Card>
        ) : (
          <FlatList
