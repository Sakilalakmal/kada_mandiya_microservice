import type { Request, Response } from "express";
import { z } from "zod";
import { listNotifications, markAllNotificationsRead, markNotificationRead } from "../repositories/notification.repo";

function requireVendorId(req: Request, res: Response): string | null {
  const vendorId = req.vendor?.vendorId;
  if (!vendorId) {
    res.status(401).json({
      error: { code: "UNAUTHORIZED", message: "Missing authenticated vendor." },
    });
    return null;
  }
  return vendorId;
}

const ListQuerySchema = z
  .object({
    unreadOnly: z.enum(["true", "false"]).optional().default("false"),
    page: z.coerce.number().int().min(1).default(1),
    pageSize: z.coerce.number().int().min(1).max(50).default(10),
  })
  .strict();

const NotificationIdSchema = z.string().uuid();

export async function getVendorNotifications(req: Request, res: Response) {
  try {
    const vendorId = requireVendorId(req, res);
    if (!vendorId) return;

    const parsed = ListQuerySchema.safeParse(req.query ?? {});
    if (!parsed.success) {
      return res.status(400).json({
        error: {
          code: "VALIDATION_ERROR",
          message: "Invalid query params.",
          details: parsed.error.flatten(),
        },
      });
    }

    const { total, unreadCount, notifications } = await listNotifications({
      recipientType: "VENDOR",
      recipientId: vendorId,
      unreadOnly: parsed.data.unreadOnly === "true",
      page: parsed.data.page,
      pageSize: parsed.data.pageSize,
    });

    return res.json({
      ok: true,
      page: parsed.data.page,
      pageSize: parsed.data.pageSize,
      total,
      unreadCount,
      notifications,
    });
  } catch (err) {
    console.error("getVendorNotifications error:", err);
    return res.status(500).json({ ok: false, message: "Server error" });
  }
}

export async function markVendorAllRead(req: Request, res: Response) {
  try {
    const vendorId = requireVendorId(req, res);
    if (!vendorId) return;

    const updatedCount = await markAllNotificationsRead({ recipientType: "VENDOR", recipientId: vendorId });
    return res.json({ ok: true, updatedCount });
  } catch (err) {
    console.error("markVendorAllRead error:", err);
    return res.status(500).json({ ok: false, message: "Server error" });
  }
}

export async function markVendorNotificationRead(req: Request, res: Response) {
  try {
    const vendorId = requireVendorId(req, res);
    if (!vendorId) return;

    const parsed = NotificationIdSchema.safeParse(req.params.id);
    if (!parsed.success) {
      return res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Invalid notification id." } });
    }

    const result = await markNotificationRead({
      recipientType: "VENDOR",
      recipientId: vendorId,
      notificationId: parsed.data,
    });

    if (result === "not_found") {
      return res.status(404).json({ error: { code: "NOT_FOUND", message: "Notification not found." } });
    }

    return res.json({ ok: true });
  } catch (err) {
    console.error("markVendorNotificationRead error:", err);
    return res.status(500).json({ ok: false, message: "Server error" });
  }
}

