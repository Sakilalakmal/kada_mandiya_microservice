...
export function getRabbitUrl(): string {
  ...
  if (!prodUrlWarningPrinted) {
    ...
    if (env === "production") {
      if (/localhost|127\.0\.0\.1/i.test(resolved)) {
        console.warn(`[notification-service] RABBITMQ_URL points to localhost (DEV-ONLY): ${resolved}`);
      }
      if (/guest:guest@/i.test(resolved)) {
        console.warn("[notification-service] RABBITMQ_URL appears to use guest/guest (DEV-ONLY).");
      }
    }
  }
  ...
  if (!devUrlWarningPrinted) {
    ...
    if (env && env !== "development" && env !== "test") {
      console.warn(
        `[notification-service] RABBITMQ_URL is not set; falling back to ${fallback} (DEV-ONLY default)`
      );
    }
  }
  return fallback;
