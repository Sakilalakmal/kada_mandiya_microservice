...
import { useVendorUnreadCountQuery } from "@/features/notifications/queries";
import { NotificationsPopoverPanel } from "./notifications-popover-panel";

export function VendorNotificationBell({ className }: { className?: string }) {
  const { token, isVendor } = useAuth();
  const unreadQuery = useVendorUnreadCountQuery();
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="p-0">
        {open ? <NotificationsPopoverPanel scope="vendor" onClose={() => setOpen(false)} /> : null}
      </PopoverContent>
    </Popover>
  );
}
